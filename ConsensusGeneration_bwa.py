from ConsensusModule_bwa import *
import sys,os,gzip,numpy

#check args
if len(sys.argv) < 4:
	sys.stderr.write("usage: %s output_directory RepeatCutoff ReadLength file_1.fastq file_2.fastq ... file_n.fastq\n" % (sys.argv[0]))
	sys.exit(1)

#validate input files
infiles = sys.argv[4:]
workdir = sys.argv[1]
RepeatCutoff = int(sys.argv[2])
...
for f in infiles:
	...
	#fin = gzip.open(f,'rb')
	#totalseq = len(fin.read().split('\n'))/4
	#fin.close()
	infile = gzip.open(f, 'rb')
	#counter_PoorQuality, counter_NoRepeats, counter_AbnormalRepeatLength, counter_LowIdentity, counter_ConsensusSequences, counter_TotalReads, counter_RepeatLengths = Consensus(infile, outfile, RepeatCutoff, ReadLength, totalseq)
	counter_PoorQuality, counter_NoRepeats, counter_AbnormalRepeatLength, counter_LowIdentity, counter_ConsensusSequences, counter_TotalReads, counter_RepeatLengths = Consensus(infile, outfile, RepeatCutoff, ReadLength)
	infile.close()
